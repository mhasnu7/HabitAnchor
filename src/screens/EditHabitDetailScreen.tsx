...
type EditHabitDetailScreenProps = NativeStackScreenProps<RootStackParamList, 'EditHabitDetail'>;
...
const EditHabitDetailScreen = ({ navigation, route }: EditHabitDetailScreenProps) => {
  ...
  const [targetCompletionDate, setTargetCompletionDate] = useState(habitToEdit?.targetCompletionDate || '');
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { backgroundColor: theme.background }]}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Edit {habitToEdit.name}</Text>
        <View style={styles.headerRightPlaceholder} />
      </View>

      <ScrollView contentContainerStyle={styles.content}>
        {/* Habit Icon and Color Display */}
        <View style={styles.iconDisplayContainer}>
          <View style={[styles.habitIconContainer, { backgroundColor: color, borderRadius: 14 }]}>
        ...
        <View style={[styles.settingItem, { backgroundColor: theme.cardBackground }]}>
          ...
          <TextInput
            style={[styles.textInput, { color: theme.text, borderBottomColor: theme.subtleText }]}
            value={name}
        ...
        <View style={[styles.settingItem, { backgroundColor: theme.cardBackground }]}>
          <Text style={[styles.settingLabel, { color: theme.text }]}>Target Completion Date (YYYY-MM-DD)</Text>
          <TextInput
            style={[styles.textInput, { color: theme.text, borderBottomColor: theme.subtleText }]}
